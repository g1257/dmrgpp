...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.1cm}p{3.4cm}p{7.5cm}}
    ...
    \addlinespace
    Fig.\ 3 diagnostic runtime &
    Long-running, not a crash regression &
    \texttt{inputNeqGBEKFig3L3.ain} is a 100-step near-atomic diagnostic
    with a five-site fitted first bath. A current five-minute run timed out
    without reproducing the historical step-3 crash; use the focused fast
    CTest for routine regression coverage. \\
    \bottomrule
  \end{tabular}
\end{table}
