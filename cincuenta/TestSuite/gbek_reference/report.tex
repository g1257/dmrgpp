...
%
% \DEVNOTES toggles the "bug found and fixed" development-history callouts
...
% full internal version. For a status-only copy, build report-summary.tex.
\documentclass[11pt]{article}
\providecommand{\DEVNOTES}{0}
\newif\ifdevnotes
\ifnum\DEVNOTES=0 \devnotestrue \else \devnotesfalse \fi
...
paper's own Figs.\ 3, 4, 7, 8, 9, and 10, and how the C++ production
solver's atomic-limit rank-3 reconstruction compares with the independent
Python reference.
\end{minipage}
\end{center}
...
\begin{itemize}
  ...
  \item \textbf{Both decomposition schemes.} The causal, row-by-row
    \textbf{Cholesky} decomposition of the hybridization target Lambda (paper Eqs.\
...
non-causal (eigenvector) decomposition reconstructs the exact hybridization target Lambda,
...
\begin{figure}[h]
  ...
  \caption*{\textbf{Paper} --- input hybridization target Lambda, rank-2/3 Cholesky and
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \caption*{\textbf{Ours} --- independent atomic-limit reference vs.\
      the production C++ solver's atomic-limit rank-3 Cholesky
      reconstruction: 2D map, residual, and fixed-$t'$ slices.}
  \end{subfigure}
\end{figure}
...
\begin{table}[h]
  ...
  \begin{tabular}{ccc}
    \toprule
    $U$ & $L_\mathrm{bath}$ & $\max|\Etot(t)-\Etot(t_q)|$ for $t>t_q$ \\
    \midrule
    2 & 4 & 0.291 \\
    2 & 6 & 0.197 \\
    2 & 8 & 0.038 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.1cm}p{3.4cm}p{7.5cm}}
    ...
    Absent from report validation &
    Every paper-figure and C++ comparison regenerated for this report uses
    \texttt{NeqAtomicLimit=1}, hence $\Lambda^-=0$ and no fitted first
    bath. The broader fitted-first-bath handoff remains covered separately
    by fast and Nightly CTests. \\
    \addlinespace
...
\begin{itemize}
  \item \textbf{Figs.\ 3, 4, 7, 8, 9, 10} --- all reproduced from the
    atomic-limit Python reference to close quantitative/qualitative
    agreement with the paper; Fig.\ 3 additionally compares directly with
    the production C++ atomic-limit reconstruction.
  \ifdevnotes
  ...
  \item \textbf{C++ Fig.\ 3 parity} --- the production solver's own
    ...
    hybridization target Lambda)\fi.
  \item \textbf{Next}: consider testing $L=5$ now that Lanczos seeding is
    fixed; arbitrary (non-half) filling remains unimplemented.
\end{itemize}
