...
void apply_Htarget_sparse( 
                    ...
                    IntegerType nC_[],
                    FpType *gAbatch_[], IntegerType ld_gAbatch_[],
                    FpType *gBbatch_[], IntegerType ld_gBbatch_[],
 ...
 if (pBXmem == NULL) {
    ...
    for(ipatch=1; ipatch <= npatches; ipatch++) {
       printf("ld_gBbatch(%d)=%d, ld_gAbatch(%d)=%d\n",
                 ipatch, ld_gBbatch_[ipatch-1],
                 ipatch, ld_gAbatch_[ipatch-1] );
       };
   };  
 assert( pBXmem != NULL );
  ...
  for(ipatch=1; ipatch <= npatches; ipatch++) {
    ...
    for(jpatch=1; jpatch <= npatches; jpatch++) {
        ...
        for(iconnection=1; iconnection <= nconnection; iconnection++) {
           /*
            ------------------------------------------------------------------
            BX(1:nrowBX,1:ncolBX) = B(1:nrowB, 1:ncolB) * XJ(1:nrowX, 1:ncolX)
            ------------------------------------------------------------------
            */
            char transA = 'N';
            char transB = 'N';
