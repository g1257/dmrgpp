...
void dmrg_set_readonly( void *devPtr, size_t nbytes, IntegerType device )
{
  if (dmrg_is_managed(devPtr)) { 
	...
	cudaError_t istat = cudaMemAdvise( devPtr, nbytes, cudaMemAdviseSetReadMostly, device );
...
void dmrg_unset_readonly( void *devPtr, size_t nbytes, IntegerType device )
{
  if (dmrg_is_managed(devPtr)) { 
	...
	cudaError_t istat = cudaMemAdvise( devPtr, nbytes, cudaMemAdviseUnsetReadMostly, device );
	if (istat != cudaSuccess) {
		fprintf(stderr,"dmrg_set_readonly: %s\n", cudaGetErrorString(istat) );
	};
	assert( istat == cudaSuccess );
...
T* ptr = NULL;
   ...
   if (dmrg_get_ngpu() == 1)
   {
     ...
     size_t count = alloc_size;
     cudaError_t ierr = cudaMemAdvise( ptr, 
                                      count, 
...
void dmrg_lacpy(  const char* uplo,
                  ...
                  const T *src, const IntegerType ld_src, 
                        T *dest, const IntegerType ld_dest )
{
#ifdef USE_MAGMA
        ...
        if (is_block_copy) {
          ...
          for(jcol=0; jcol < ncol; jcol++) {
                ...
                IntegerType istart = is_upper ? 0 :
                             is_lower ? irow :
                                         0;
                IntegerType iend = is_upper ? irow :
                           is_lower ? m-1  :
                                      m-1;
...
void dmrg_prefetch_to_device( void *unified_memory_ptr, size_t nbytes, IntegerType idevice  )
{
   ...
   if (dmrg_is_managed( unified_memory_ptr) ) {
	...
	if (idevice > (ndevice -1)) { idevice  = (ndevice-1); };
...
template void dmrg_lacpy<MYTYPE>(  const char* uplo,
                  const IntegerType m, const IntegerType n,
                  const MYTYPE *src, const IntegerType ld_src,
                        MYTYPE *dest, const IntegerType ld_dest );
...
template void dmrg_lacpy<double>(  const char* uplo,
                  const IntegerType m, const IntegerType n,
                  const double *src, const IntegerType ld_src,
                        double *dest, const IntegerType ld_dest );
#else
template void dmrg_free<std::complex<double> >(std::complex<double>*);
template std::complex<double>* dmrg_malloc<std::complex<double> >(const size_t alloc_size, SizeType size);
template void dmrg_lacpy<std::complex<double> >(  const char* uplo,
                  const IntegerType m, const IntegerType n,
                  const std::complex<double> *src, const IntegerType ld_src,
                        std::complex<double> *dest, const IntegerType ld_dest );
#endif
