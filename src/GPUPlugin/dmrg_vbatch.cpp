...
{
	SizeType myint = (  (  ((ipatch)-1) + (((jpatch)-1)*(npatches))  ) + ( ((ioperator)-1)*((npatches)*(npatches)) )   );
	return myint;
}
...
void dmrg_Xgetvector( const IntegerType n, 
                      T *dx_src, const IntegerType incx,
...
void dmrg_Xgetmatrix( const IntegerType m, const IntegerType n, 
                      T *dA_src, const IntegerType ldda,
                       ...
                       IntegerType* group_size)
{
	...
	if (idebug >= 1) {
		...
		for(igroup=0; igroup < group_count; igroup++) {
			...
			gflops += ( (double) m_array[igroup]) *
			        ( (double) n_array[igroup]) *
			        ( (double) k_array[igroup]) *
...
#ifdef USE_INTEL_MKL
	{
		...
		cblas_Xgemm_batch( CblasColMajor,
		                   ...
		                   alpha_array,  (const T **) &(a_array[0]), lda_array_mkl,
		        (const T **) &(b_array[0]), ldb_array_mkl,
   ...
   */
	...
	nbytes = sizeof(T) * (vbatch_dim); nbytes_total += nbytes;
	T* alpha_vbatch = new T[vbatch_dim];

	nbytes = sizeof(T) * (vbatch_dim); nbytes_total += nbytes;
	T* beta_vbatch = new T[vbatch_dim];

	nbytes = sizeof(IntegerType) * (vbatch_dim); nbytes_total += nbytes;
	...
	*/
		...
		IntegerType is_same_all = is_same_transa &&
		        is_same_transb &&
		        is_same_alpha &&
		        is_same_beta;
	 ...
	 */
		if (idebug >= 1) {
			...
			for(i=0; i < batch_size; i++) {
				...
				IntegerType is_ok_mnk = is_ok_mm &&
				        is_ok_nn &&
				        is_ok_kk;
				if (!is_ok_mnk) {
					fprintf(stderr,
				...
				IntegerType is_ok_ldabc = is_ok_lda &&
				        is_ok_ldb &&
				        is_ok_ldc;
		...
		if (ngpu == 1) {

			{
				...
				magmablas_Xgemm_vbatched_max_nocheck( transA, transB,
				                                      ...
				                                      (MAGMA_T) alpha,
				                                      (MAGMA_T const * const *) a_vbatch, lda_vbatch,
				                                      (MAGMA_T const * const *) b_vbatch, ldb_vbatch,
				                                      (MAGMA_T) beta,
				                                      (MAGMA_T **) c_vbatch, ldc_vbatch,
				                                      batch_size,
				                                      max_m, max_n, max_k,
				                                      queue );
			...
			for(idev=0; idev < ngpu; idev++) {
				...
				if (pbatch_size >= 1) {
					magmablas_Xgemm_vbatched_max_nocheck( transA, transB,
					                                      ...
					                                      (MAGMA_T) alpha,
					                                      (MAGMA_T const * const *) pa_vbatch, plda_vbatch,
					                                      (MAGMA_T const * const *) pb_vbatch, pldb_vbatch,
					                                      (MAGMA_T) beta,
					                                      (MAGMA_T **) pc_vbatch, pldc_vbatch,
					                                      pbatch_size,
					                                      max_m, max_n, max_k,
					                                      queue );
				};
...
template void dmrg_Xgemm_vbatch<MYTYPE>(
char*,
char*,
IntegerType*,
IntegerType*,
...
template void dmrg_Xgemm_vbatch<double>(
                        char*,
                        char*,
                        IntegerType*,
                        IntegerType*,
...
#else
template void dmrg_Xgemm_vbatch<std::complex<double> >(
                        char* ctransa_array,
                        char* ctransb_array,
                        IntegerType*  m_array,
                        IntegerType*  n_array,
...
template void dmrg_Xgetvector<MYTYPE>(const IntegerType,MYTYPE*, const IntegerType, MYTYPE*, const IntegerType);
template void dmrg_Xsetvector<MYTYPE>(const IntegerType,const MYTYPE*, const IntegerType, MYTYPE*, const IntegerType);
template void dmrg_Xgetmatrix<MYTYPE>(const IntegerType,const IntegerType, MYTYPE*, const IntegerType,MYTYPE*, const IntegerType);
template void dmrg_Xsetmatrix<MYTYPE>(const IntegerType,const IntegerType,const MYTYPE*, const IntegerType,MYTYPE*, const IntegerType);
