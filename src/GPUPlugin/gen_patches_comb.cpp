...
double bincoeff(SizeType n, SizeType k) {
   return( nchoosek( n, k ) );
}



SizeType gen_patches_comb( 
          SizeType left_size,
          SizeType right_size,
          SizeType target_up, 
...
VectorSizeType isvalid_right_(ld_isvalid_right*(1+max_right_down),0);
  ...
  for(nleft_up=0;   nleft_up   <= max_left_up;   nleft_up++) {
       isvalid_left_[indx2f2(1+nleft_up,1+nleft_down,ld_isvalid_left)] = 0;
       };
       };

  for(nright_down=0; nright_down <= max_right_down; nright_down++) {
  for(nright_up=0;   nright_up   <= max_right_up;   nright_up++) {
       isvalid_right_[indx2f2(1+nright_up,1+nright_down,ld_isvalid_right)] = 0;
       };
       };
   }
...
for(nleft_up=0;   nleft_up   <= max_left_up;   nleft_up++) {
    ...
    SizeType isvalid = (0 <= nright_up) && (nright_up <= max_right_up) && 
                  (0 <= nright_down) && (nright_down <= max_right_down);
    isvalid_left_[indx2f2(1+nleft_up,1+nleft_down,ld_isvalid_left)] = isvalid ? 1 : 0;
    isvalid_right_[indx2f2(1+nright_up,1+nright_down,ld_isvalid_right)] = isvalid ? 1 : 0;
 };
 };
...
*/
  ...
  for(nleft_down=0; nleft_down <= max_left_down; nleft_down++) {
  for(nleft_up=0;   nleft_up   <= max_left_up;   nleft_up++) {
     SizeType isvalid = isvalid_left_[indx2f2( 1+nleft_up,1+nleft_down,ld_isvalid_left)];
     if (isvalid) {  
        isum  = isum + 1;
        };
     };
...
std::vector<double> right_ways_(ld_right_ways*(max_right_down+1),0.0);
  ...
  for(nright_down=0; nright_down <= max_right_down; nright_down++) {
  for(nright_up=0; nright_up <= max_right_up; nright_up++) {
     right_ways_[indx2f2(1+nright_up,1+nright_down,ld_right_ways)] = 0;
     };
     };
   }
...
std::vector<double> Cmat_left_(ld_Cmat_left*(1+max_left_down),0.0);
  ...
  for( nleft_down = 0; nleft_down <= max_left_down; nleft_down++) {
  for( nleft_up = 0; nleft_up <= max_left_up; nleft_up++) {
     Cmat_left_[indx2f2(1+nleft_up,1+nleft_down,ld_Cmat_left)] = C_nleft_up_[nleft_up] * 
                                           C_nleft_down_[nleft_down];
     };
     };
...
*/
  ...
  for(nright_down=0; nright_down <= max_right_down; nright_down++) {
  for(nright_up=0; nright_up <= max_right_up; nright_up++) {
     Cmat_right_[ indx2f2(1+nright_up,1+nright_down,ld_Cmat_right)] = C_nright_up_[nright_up] * 
                                              C_nright_down_[nright_down];
  };
  };
  ...
  for(nright_up=0; nright_up <= max_right_up; nright_up++) {
   ...
   if (isvalid_right_[indx2f2(1+nright_up,1+nright_down,ld_isvalid_right)]) {
/*
%    ---------------------------------
%    note, beware of possible overflow
%    or loss of accuracy
...
double total_right_ways = 0;
  ...
  for(nleft_down=0; nleft_down <= max_left_down; nleft_down++) {
  for(nleft_up=0; nleft_up <= max_left_up; nleft_up++) {
    total_left_ways += left_ways_[indx2f2(1+nleft_up,1+nleft_down,ld_left_ways)];
    };
    };

  for(nright_down=0; nright_down <= max_right_down; nright_down++) {
  for(nright_up=0; nright_up <= max_right_up; nright_up++) {
    total_right_ways += right_ways_[indx2f2(1+nright_up,1+nright_down,ld_right_ways)];
    };
    };
...
*/
  ...
  for(nleft_up=0; nleft_up <= max_left_up; nleft_up++) {
  for(nleft_down=0; nleft_down <= max_left_down; nleft_down++) {
     double frac = ( left_ways_[indx2f2(1+nleft_up,1+nleft_down,ld_left_ways)]/total_left_ways * keep_left_states );

     left_ways_[indx2f2(1+nleft_up,1+nleft_down,ld_left_ways)] =  (use_ceil) ? ceil( frac ) : round(frac);
     };
     };

  for(nright_up=0; nright_up <= max_right_up; nright_up++) {
  for(nright_down=0; nright_down <= max_right_down; nright_down++) {
     double frac = ( right_ways_[indx2f2(1+nright_up,1+nright_down,ld_right_ways)]/total_right_ways * keep_right_states );

     right_ways_[indx2f2( 1+nright_up,1+nright_down,ld_right_ways)] =  (use_ceil) ? ceil( frac ) : round(frac);
     };
     };
...
*/
  ...
  for (nleft_up=0; nleft_up <= max_left_up; nleft_up++) {
    ...
    if (isvalid_left_[indx2f2(1+nleft_up,1+nleft_down,ld_isvalid_left)]) {
     SizeType lsize = left_ways_[indx2f2(1+nleft_up,1+nleft_down,ld_left_ways)];
     SizeType rsize = right_ways_[indx2f2(1+nright_up,1+nright_down,ld_right_ways)];
