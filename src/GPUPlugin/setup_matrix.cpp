...
#else
double makeFloat(double zr,double)
{
	return zr;
}
#endif
...
//   T* Abatch = new T[nrow_Abatch*ncol_Abatch*noperator];
   ...
   for(ipatch=1; ipatch <= npatches; ipatch++) {
      ...
      T* Amat_ = &(Abatch[f1(ipatch,jpatch,ioperator,nrow_Abatch,ncol_Abatch)]);
      ...
      ld_Amatrix_[f3(ipatch, jpatch, ioperator, npatches)] = left_patch_size_[ipatch-1];
      ...
      for(i=1; i <= nrow; i++) {
         ...
         dval += ( (double) (ioperator-1)) *
                 ...
                 ( (double) total_left_size);
	   Amat_[ ((i)-1) + ((j)-1)*ld_Amat] = makeFloat(dval,-dval);
         };
         };
...
SizeType f1(const SizeType& ipatch, 
	...
	const SizeType& ncol_Abatch) 
{
  SizeType mySizeType = (((ipatch)-1)+((jpatch)-1)*nrow_Abatch) + ((long)((ioperator)-1)*(nrow_Abatch) * (ncol_Abatch));
  return mySizeType;
}
...
SizeType f2(const SizeType& ipatch,
	...
	const SizeType& npatches) 
{
  SizeType mySizeType = (((ipatch)-1) + ((jpatch)-1)*npatches) + ((ioperator)-1)*( npatches ) *(npatches);
  return mySizeType;
}
