               ...
               VectorSizeType& gnnz_B_)
{
	...
	for(ioperator=1; ioperator <= noperator; ioperator++) {
		for(jpatch=1; jpatch <= npatches; jpatch++) {
			for(ipatch=1; ipatch <= npatches; ipatch++) {
				...
				SizeType ld_Amat = ld_Amatrix_[index3(ipatch,jpatch,ioperator,npatches)];
				SizeType ld_Bmat = ld_Bmatrix_[index3(ipatch,jpatch,ioperator,npatches)];
	   ...
	   */
				if (Amat_ == NULL) {
					...
					for(ja=1; ja <= ncolA; ja++) {
						for(ia=1; ia <= nrowA; ia++) {
							...
							nnz_Amat += ((Amat_[indx2f(ia,ja,ld_Amat)] == T(0)) ? 0 : 1);
						};
					};
				};
				...
				if (Bmat_ == NULL) {
					...
					for(jb=1; jb <= ncolB; jb++) {
						for(ib=1; ib <= nrowB; ib++) {
							nnz_Bmat += ((Bmat_[indx2f(ib,jb,ld_Bmat)] == T(0)) ? 0 : 1);
						};
					};
				};
		  ...
		  */
		  ...
		  ------------------------------------------
		  */
					nC_[indx2f(ipatch,jpatch,npatches)] = nC_[indx2f(ipatch,jpatch,npatches)] + 1;
				};
			}; /* end for jpatch */
		}; /* end for ipatch */
	}; /* end for ioperator */
	...
	if (idebug >= 1) {
		...
		printf("setup_nC:npatches=%lu,noperator=%lu,total_number=%lu\n",
		       npatches,   noperator,   total_number);

	};
}

template void setup_nC<MYTYPE>(
		SizeType,
              SizeType,
               /*
                * ---------
