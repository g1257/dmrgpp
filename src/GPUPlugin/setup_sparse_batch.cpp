...
*/
{
	...
	if (idebug >= 1) {
		printf("setup_sparse_batch:npatches=%lu,noperator=%lu,",
		       npatches,   noperator );
		...
		for(ipatch=1; ipatch <= npatches; ipatch++) {
			...
			for(jpatch=1; jpatch <= npatches; jpatch++) {
				for(ioperator=1; ioperator <= noperator; ioperator++) {
					...
					SizeType nnz_Amat = gnnz_A_[index3(ipatch,jpatch,ioperator,npatches)];
					SizeType nnz_Bmat = gnnz_B_[index3(ipatch,jpatch,ioperator,npatches)];
					...
					if (has_work) {
						...
						IntegerType m = nrowA;
						IntegerType n = ncolA;
						IntegerType ld1 = ld_Amatrix_[index3(ipatch,jpatch,ioperator,npatches)];
						IntegerType ld2 = ld_gAbatch_[ipatch-1];

						IntegerType isok = (1 <= m) && (1 <= n) && (m <= ld1) && (m <= ld2);
						assert( isok );


						if (use_Xlacpy) {
							Xlacpy_(  uplo, &m, &n, Asrc, &ld1, Adest, &ld2);
						}
						else {
					...
					if (has_work) {
						...
						IntegerType m = nrowB;
						IntegerType n = ncolB;
						IntegerType ld1 = ld_Bmatrix_[index3(ipatch,jpatch,ioperator,npatches)];
						...
						if (use_Xlacpy) {
							Xlacpy_(  uplo, &m, &n, Bsrc, &ld1, Bdest, &ld2);
						}
						else {
...
template<typename T>
void unsetup_sparse_batch( T*** pgAbatch, T*** pgBbatch)
{	
...
template void unsetup_sparse_batch<double>(double***,
					   double***);
#else
