		...
		VectorSizeType gnnz_B_)
{
 ...
 for(ipatch=1; ipatch <= npatches; ipatch++) {
    fprintf(fid_A,"left_patch_size(%lu) = %lu;\n", ipatch,left_patch_size_[(ipatch)-1]);
    fprintf(fid_B,"right_patch_size(%lu) = %lu;\n", ipatch,right_patch_size_[(ipatch)-1]);
    };
   ...
   for(jpatch=1; jpatch <= npatches; jpatch++) {
     for(ipatch=1; ipatch <= npatches; ipatch++) {
       if (gnnz_A_[index3(ipatch,jpatch,ioperator,npatches)] > 0) {
         fprintf( fid_A,"gnnz_A(%lu,%lu,%lu) = %lu;\n", 
                  ipatch,jpatch,ioperator,gnnz_A_[index3(ipatch,jpatch,ioperator,npatches)]);
         };

       if (gnnz_B_[index3(ipatch,jpatch,ioperator,npatches)] > 0) {
         fprintf( fid_B,"gnnz_B(%lu,%lu,%lu) = %lu;\n", 
                  ipatch,jpatch,ioperator,gnnz_B_[index3(ipatch,jpatch,ioperator,npatches)]);
         };
       };
     };
   };
...
template<typename T>
void setup_vbatch( SizeType noperator,    /* number of connections (INPUT) */
...
//  std::vector<T> Bbatch_(ld_Abatch*ncol_Abatch,T(0.0));
  ...
  for(ipatch=1; ipatch <= npatches; ipatch++) {
    left_patch_start_[ipatch] = left_patch_start_[(ipatch)-1] + left_patch_size_[(ipatch)-1];
    };

  right_patch_start_[0] = 1;
  for(ipatch=1; ipatch <= npatches; ipatch++) {
    right_patch_start_[ipatch] = right_patch_start_[(ipatch)-1] + right_patch_size_[(ipatch)-1];
    };
  ...
  */
  ...
  for(SizeType i=0; i < size_Abatch; i++) {
   ...
   for(jpatch=1; jpatch <= npatches; jpatch++) {
   for(ipatch=1; ipatch <= npatches; ipatch++) {
      T *Asrc = Amatrix_[index3(ipatch,jpatch,ioperator,npatches)];
      SizeType ld_Asrc = ld_Amatrix_[index3(ipatch,jpatch,ioperator,npatches)];
      ...
      T* Adest = hAbatch_[indx2f(ia,ja + (ioperator-1)*left_max_state,ld_Abatch)]; 
      ...
      SizeType isok = (1 <= m) &&
                 ...
                 (m <= ld2);
      if (!isok) {
       printf("setup_vbatch:ipatch=%lu,jpatch=%lu,ioperator=%lu\n",
                            ipatch, jpatch, ioperator );
       printf("npatches=%lu,noperator=%lu, left_max_state=%lu\n",
               (SizeType) npatches, (SizeType) noperator,(SizeType) left_max_state );
      ...
      //  ------------------------------------------
      //  copy m by n submatrix from Asrc  to Adest
      //  ------------------------------------------
      ...
      if (idebug >= 1) {
        ...
        if (ipatch == jpatch) {
           nnz_Adiag[ioperator-1] += lnnz_A;
           };

        gnnz_A_[index3(ipatch,jpatch,ioperator,npatches)] = lnnz_A;
        };
  ...
  for(SizeType i=0; i < size_Bbatch; i++) {
   ...
   for(ioperator=1; ioperator <= noperator; ioperator++) {
     ...
     for(jpatch=1; jpatch <= npatches; jpatch++) {
       ...
       T *Bsrc = Bmatrix_[index3(ipatch,jpatch,ioperator,npatches)];
       SizeType ld_Bsrc = ld_Bmatrix_[index3(ipatch,jpatch,ioperator,npatches)];
       ...
       T *Bdest = hBbatch_[indx2f(ib,jb + (ioperator-1)*right_max_state,ld_Bbatch)];
       ...
       if (idebug >= 1) {
          ...
          if (ipatch == jpatch) {
              nnz_Bdiag[ioperator-1] += lnnz_B;
              };
          gnnz_B_[index3(ipatch,jpatch,ioperator,npatches)] = lnnz_B;
          };
   ...
   */
     ...
     for( ipatch=1; ipatch <= npatches; ipatch++) {
          ...
          printf("npatches=%lu, noperator=%lu left_max_state=%lu right_max_state=%lu\n", 
                  npatches,    noperator,   left_max_state,   right_max_state);

          {
          SizeType ioperator = 0;
          for(ioperator=0; ioperator < noperator; ioperator++) {

            double sparsity_ratio_A = ((nnz_A[ioperator]/left_max_state)/left_max_state);
            double sparsity_ratio_B = ((nnz_B[ioperator]/right_max_state)/right_max_state);
            ...
            printf("sparsity ratio A=%6.4f, fraction in diag blocks=%6.4f ", 
                    sparsity_ratio_A,       fraction_in_diag_A );
            printf("sparsity ratio B=%6.4f, fraction in diag blocks=%6.4f ", 
                    sparsity_ratio_B,       fraction_in_diag_B );
            printf(" \n");
          ...
          printf("total_flops_method_1=%le (%6.2lf), ",
                  total_flops_method_1,     total_flops_method_1/total_flops_min );
          printf("total_flops_method_2=%le (%6.2lf), ",
                  total_flops_method_2,     total_flops_method_2/total_flops_min);
...
int ICEIL2(const SizeType& x,const SizeType& n)
{
	return	(( (x) + (n)-1 )/(n));
}
/*
...
*/
template void setup_vbatch<MYTYPE>( SizeType noperator,    /* number of connections (INPUT) */
                   SizeType npatches,     /* number of patches (INPUT) */
                   const VectorSizeType& left_patch_size_, /* sizes of left patches (INPUT) */
                   const VectorSizeType& right_patch_size_, /* sizes of right patches (INPUT) */
                   VectorSizeType& left_patch_start_, /* cumulative sum  of left_patch_size (OUTPUT) */
