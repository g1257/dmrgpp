...
template void csr_den_kron_mult_method<RealType>(const int                                 imethod,
                                                 const char                                transA,
                                                 const char                                transB,
...
template bool csr_is_eye<RealType>(const PsimagLite::CrsMatrix<RealType>&);

template void csr_transpose<RealType>(const int      nrow_A,
                                      const int      ncol_A,
                                      const int      arowptr[],
                                      const int      acol[],
                                      const RealType aval[],
...
template void csr_kron_mult_method<RealType>(const int  imethod,
                                             const char transA,
                                             const char transB,
...
template void csr_matmul_post<RealType>(const char trans_A,
...
template void csr_matmul_pre<RealType>(const char trans_A,
                                       const PsimagLite::CrsMatrix<RealType>&,
                                       const int                                         nrow_Y,
                                       const int                                         ncol_Y,
                                       const PsimagLite::MatrixNonOwned<const RealType>& yin,
...
template void csr_submatrix<RealType>(const PsimagLite::CrsMatrix<RealType>& a,
                                      const int                              nrow_B,
                                      const int                              ncol_B,
                                      const int                              max_nnz,
...
template void csc_matmul_pre<RealType>(const char                                trans_A,
                                       const int                                 nrow_A,
                                       const int                                 ncol_A,
...
template void csc_matmul_post<RealType>(const char                                trans_A,
                                        const int                                 nrow_A,
                                        const int                                 ncol_A,
                                        const PsimagLite::Vector<int>::Type&      acolptr,
                                        const PsimagLite::Vector<int>::Type&      arow,
...
template void csc_kron_mult_method<RealType>(const int                                 imethod,
                                             const int                                 nrow_A,
                                             const int                                 ncol_A,
                                             const PsimagLite::Vector<int>::Type&      acolptr,
                                             const PsimagLite::Vector<int>::Type&      arow,
...
template void csc_kron_mult<RealType>(const int                                 nrow_A,
                                      const int                                 ncol_A,
                                      const PsimagLite::Vector<int>::Type&      acolptr,
                                      const PsimagLite::Vector<int>::Type&      arow,
                                      const PsimagLite::Vector<RealType>::Type& aval,
...
template void coord2csr<RealType>(const int      nrow_A,
                                  const int      ncol_A,
                                  const int      nnz,
                                  const int      ilist[],
                                  const int      jlist[],
...
template void den_csr_kron_mult_method<RealType>(const int                                 imethod,
                                                 const char                                transA,
                                                 const char                                transB,
...
template void den_matmul_pre<RealType>(const char                                        trans_A,
                                       const int                                         nrow_A,
                                       const int                                         ncol_A,
                                       const PsimagLite::Matrix<RealType>&               a_,
                                       const int                                         nrow_Y,
...
template void den_matmul_post<RealType>(const char                                        trans_A,
                                        const int                                         nrow_A,
                                        const int                                         ncol_A,
                                        const PsimagLite::Matrix<RealType>&               a_,
                                        const int                                         nrow_Y,
...
template void den_kron_submatrix<RealType>(const int                            nrow_A,
                                           const int                            ncol_A,
                                           const PsimagLite::Matrix<RealType>&  a_,
                                           const int                            nrow_B,
                                           const int                            ncol_B,
...
template void den_kron_mult_method<RealType>(const int                                 imethod,
                                             const char                                transA,
                                             const char                                transB,
                                             const PsimagLite::Matrix<RealType>&       a_,
                                             const PsimagLite::Matrix<RealType>&       b_,
...
template int den_nnz<RealType>(const PsimagLite::Matrix<RealType>&);

template bool den_is_eye<RealType>(const PsimagLite::Matrix<RealType>&);

template bool den_is_zeros<RealType>(const PsimagLite::Matrix<RealType>&);

template void den_kron_form<RealType>(const int                           nrow_A,
                                      const int                           ncol_A,
                                      const PsimagLite::Matrix<RealType>& a_,
                                      const int                           nrow_B,
                                      const int                           ncol_B,
