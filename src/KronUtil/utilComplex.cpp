...
template void csr_den_kron_mult_method<std::complex<RealType>>(
    const int  imethod,
    const char transA,
    const char transB,
...
template bool
csr_is_eye<std::complex<RealType>>(const PsimagLite::CrsMatrix<std::complex<RealType>>&);
...
template void csr_kron_mult_method<std::complex<RealType>>(
    const int  imethod,
    const char transA,
    const char transB,
...
template void csr_matmul_post<std::complex<RealType>>(
    const char trans_A,
...
template void csr_matmul_pre<std::complex<RealType>>(
    const char trans_A,
    const PsimagLite::CrsMatrix<std::complex<RealType>>&,
    const int                                                       nrow_Y,
    const int                                                       ncol_Y,
...
template void
csr_submatrix<std::complex<RealType>>(const PsimagLite::CrsMatrix<std::complex<RealType>>& a,
                                      const int                                            nrow_B,
                                      const int                                            ncol_B,
                                      const int                                            max_nnz,
...
template void
csr_kron_submatrix<std::complex<RealType>>(const PsimagLite::CrsMatrix<std::complex<RealType>>& a,
                                           const PsimagLite::CrsMatrix<std::complex<RealType>>& b,
                                           const int                                      nrindex,
                                           const int                                      ncindex,
                                           const int                                      max_nnz,
...
template void
csc_matmul_pre<std::complex<RealType>>(const char                           trans_A,
                                       const int                            nrow_A,
                                       const int                            ncol_A,
...
template void csc_matmul_post<std::complex<RealType>>(
    const char                                              trans_A,
    const int                                               nrow_A,
    const int                                               ncol_A,
    const PsimagLite::Vector<int>::Type&                    acolptr,
...
template void csc_kron_mult_method<std::complex<RealType>>(
    const int                                               imethod,
    const int                                               nrow_A,
    const int                                               ncol_A,
    const PsimagLite::Vector<int>::Type&                    acolptr,
...
template void
csc_kron_mult<std::complex<RealType>>(const int                            nrow_A,
                                      const int                            ncol_A,
                                      const PsimagLite::Vector<int>::Type& acolptr,
                                      const PsimagLite::Vector<int>::Type& arow,
...
template void den_csr_kron_mult_method<std::complex<RealType>>(
    const int                                               imethod,
    const char                                              transA,
    const char                                              transB,
...
template void
den_gen_matrix<std::complex<RealType>>(const int                                   nrow_A,
                                       const int                                   ncol_A,
                                       const RealType&                             threshold,
                                       PsimagLite::Matrix<std::complex<RealType>>& a_);

template void den_matmul_pre<std::complex<RealType>>(
    const char                                                      trans_A,
    const int                                                       nrow_A,
    const int                                                       ncol_A,
    const PsimagLite::Matrix<std::complex<RealType>>&               a_,
...
template void den_matmul_post<std::complex<RealType>>(
    const char                                                      trans_A,
    const int                                                       nrow_A,
    const int                                                       ncol_A,
    const PsimagLite::Matrix<std::complex<RealType>>&               a_,
...
template void
den_kron_submatrix<std::complex<RealType>>(const int                                         nrow_A,
                                           const int                                         ncol_A,
                                           const PsimagLite::Matrix<std::complex<RealType>>& a_,
                                           const int                                         nrow_B,
                                           const int                                         ncol_B,
...
template void den_kron_mult_method<std::complex<RealType>>(
    const int                                               imethod,
    const char                                              transA,
    const char                                              transB,
    const PsimagLite::Matrix<std::complex<RealType>>&       a_,
...
template bool
den_is_zeros<std::complex<RealType>>(const PsimagLite::Matrix<std::complex<RealType>>&);

template void
den_kron_form<std::complex<RealType>>(const int                                         nrow_A,
                                      const int                                         ncol_A,
                                      const PsimagLite::Matrix<std::complex<RealType>>& a_,
                                      const int                                         nrow_B,
                                      const int                                         ncol_B,
...
template void
den_kron_form_general<std::complex<RealType>>(const char transA,
                                              const char transB,
                                              const int  nrow_A,
                                              const int  ncol_A,
                                              const PsimagLite::Matrix<std::complex<RealType>>& a_,
...
template void
den_submatrix<std::complex<RealType>>(const int                                         nrow_A,
                                      const int                                         ncol_A,
                                      const PsimagLite::Matrix<std::complex<RealType>>& a_,
                                      const int                                         nrindex,
                                      const int                                         ncindex,
